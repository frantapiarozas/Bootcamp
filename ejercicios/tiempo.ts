export function aMinutos(x: number): number {
  return x * 60;
};
export function aHoras(x: number): number {
  return x / 60;
};
export function aHHMM(x: number): string {
  const h: number = Math.floor(x);
  const m: number = Math.round((x - h) * 60);
  return `${h < 10 ? "0" + h : h}:${m < 10 ? "0" + m : m}`;
}
export function comparar(x: number, y: number): number {
    if (x < y) return -1;
    if (x == y) return 0;
    return 1;
}
